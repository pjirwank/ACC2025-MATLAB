...
%             fprintf('case 2 \n')
        ...
        noisyRy_EKF = fun_axisangle(noisyAngle_EKF, axis);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % noisyAngvel_EKF = noisyAngvel_magnitude * noisyAngvel_direction;
        % 
%         noisyAngvel_direction = rand(3,1);
%         noisyAngvel_direction = noisyAngvel_direction/norm(noisyAngvel_direction);
%         noisyAngvel_magnitude = -maxNoisyAngvelMagnitude + 2*maxNoisyAngvelMagnitude*rand();
%         noisyAngvel = noisyAngvel_magnitude * noisyAngvel_direction;
    ...
    Rhat_EKF = Rhat_EKF_array(:,:,i);

    Ry_EKF = noisyRy_EKF * R;
    Omegay_EKF = Omega + noisyAngvel_EKF;
    ...
    [Rhat_EKF_next, P_next] = fun_EKF(Rhat_EKF, P_array(:,:,i), Ry_EKF, Omegay_EKF, cov_Ry, h);
    ...
    Rtilde_EKF_next = Rhat_EKF_next' * R_next;
    Rbar_EKF_next = Rhat_EKF_next * R_next';
    ...
    potential_Rbar_EKF(i+1) = fun_potential(Rbar_EKF_next);
...
n = number_of_jumps*3 + 3; % total number of plots
...
% % hold on
plot(time, potential_Rbar_EKF, 'r', 'LineWidth', 2)
ax = gca;
ax.FontSize = 20;
...
% Create an empty cell array for the legend labels
legend_labels = cell(1, n);

% Set specific legends as per the requirement
legend_labels{1} = 'Hybrid Filter';    % First legend
legend_labels{end-1} = 'PCF';          % One before last legend
legend_labels{end} = 'EKF';               % Empty legend
for j = 2:1:n-2
    legend_labels{j} = '';
end
