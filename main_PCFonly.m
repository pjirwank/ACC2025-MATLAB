...
% only noise in angvel measurements
maxNoisyRotationAngle = (0)*(pi/180); % rad
maxNoisyAngvelMagnitude = (1)*(pi/180); % rad/s
...
noise_angvel_magnitude_PCF = zeros(N, 1);
noise_angvel_magnitude_ECF = zeros(N, 1);
...
for i = 1:1:N-1
    ...
    elseif noise == 1
        ...
        if fun_potential(Rtilde_PCF_array(:,:,i)) > 0.999999999
%             fprintf('case 1 \n')
            ...
            noise_angvel_magnitude_PCF(i) = norm(noise_angvel_PCF);
...
%             fprintf('case 2\n')
        ...
        max_noise_angle_PCF = maxNoisyRotationAngle;%10*pi/180; %rad
        ...
        if potential_Rbar_EKF(i) < 0.999999
            noise_angvel_EKF = -sign(sin(Rbar_EKF_angle))*Rtilde_EKF_axis;
        else
            noise_angvel_EKF = sin(Rbar_EKF_angle)*Rtilde_EKF_axis;
            noise_angvel_magnitude_PCF(i) = abs(sin(Rbar_EKF_angle));
        end

        max_noise_angle = maxNoisyRotationAngle; %10*pi/180; %rad
        noise_angle_EKF = (-max_noise_angle + 2*max_noise_angle*rand());
        axis_noise_rotm = rand(3,1);
        if norm(axis_noise_rotm) < 0.001
            axis_noise_rotm = rand(3,1);
